...
SELECT task.title, task.description , user.name, user.email
FROM user_task 
...
SELECT task.title, task.description, user.name, task.created
FROM user_task
JOIN user ON user_task.user_id=user.id
JOIN task ON task.id=user_task.task_id
...
SELECT COUNT(MONTHNAME(created)) AS amount, MONTHNAME(created) AS month
FROM task
